...
def stok_islem_yap(urun_id, miktar, depo, islem="giris", ozel_tarih=None, aciklama=None):
    ...
    # KESİN ÇÖZÜM: İşlem türü transfer ise kesinlikle "TRANSFER" olarak kaydedilir (Çıkışlara/satışlara yansımaz)
    if islem == "transfer":
        tx_type = "TRANSFER"
    else:
...
def process_command(user_input):
    ...
    if (eslesen_marka or eslesen_kategori) and not is_transfer and not is_ekle and not is_cikis:
        ...
        if kat_urunleri:
            ...
            return f"İlgili depoda {grup_adi} kalmamıştır." if genel_toplam == 0 else f"{'Tüm depolar' if is_toplam else secilen_depo.upper()} toplam {sayi_formatla(genel_toplam)} {grup_adi}: " + ", ".join(detaylar)
    ...
    if is_transfer:
        if not from_depot or not to_depot or tr_temizle(from_depot) == tr_temizle(to_depot): return "Transfer depoları çakışıyor veya geçersiz."
        for item in items:
            mevcut = depo_stok_hesapla(item["prod"]["id"], from_depot)
            if mevcut < item["qty"]: hata_mesajlari.append(f"{from_depot.upper()} deposunda yeterli {item['prod']['name']} yok. Mevcut: {sayi_formatla(mevcut)}")
            elif item["qty"] > 0:
                # İki yönlü transfer kaydı: Türü "transfer" yapılarak raporda ⇄ simgesi alması sağlanır ve çıkışlara karışmaz
                stok_islem_yap(item["prod"]["id"], item["qty"], from_depot, "transfer", cozulen_tarih, f"→ {to_depot.upper()}")
                stok_islem_yap(item["prod"]["id"], item["qty"], to_depot, "transfer", cozulen_tarih, f"← {from_depot.upper()}")
                detay_mesajlari.append(f"{sayi_formatla(item['qty'])} {item['prod']['name']}")
        if hata_mesajlari: return ". ".join(hata_mesajlari)
        return f"{from_depot.upper()} -> {to_depot.upper()} transferi gerçekleştirildi ({cozulen_tarih.split()[0]}): " + ", ".join(detay_mesajlari)
        ...
        return f"[{secilen_depo.upper()}] Stok girişi yapıldı [{cozulen_tarih.split()[0]}]: " + ", ".join(detay_mesajlari)
        
    elif is_cikis:
        for item in items:
            mevcut = depo_stok_hesapla(item["prod"]["id"], secilen_depo)
            if mevcut < item["qty"]: hata_mesajlari.append(f"Depoda yeterli {item['prod']['name']} yok. Mevcut: {sayi_formatla(mevcut)}")
            elif item["qty"] > 0:
                stok_islem_yap(item["prod"]["id"], item["qty"], secilen_depo, "cikis", cozulen_tarih, None)
                detay_mesajlari.append(f"{sayi_formatla(item['qty'])} {item['prod']['name']} (Kalan: {sayi_formatla(depo_stok_hesapla(item['prod']['id'], secilen_depo))})")
        if hata_mesajlari: return ". ".join(hata_mesajlari)
        return f"[{secilen_depo.upper()}] Stok çıkışı yapıldı [{cozulen_tarih.split()[0]}]: " + ", ".join(detay_mesajlari)
...
class DummyHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Kurumsal Stok Asistani 7/24 Aktif ve Calisiyor!")
